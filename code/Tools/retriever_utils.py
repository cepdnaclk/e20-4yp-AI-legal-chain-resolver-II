...
def retrieve_chunks_title(
    ...
    logging.info(
        "Retrieving chunks for act=%s section=%s", act, section_number
    )
    doc_ids = list(vectorstore.index_to_docstore_id.values())
    documents = []
    for doc_id in doc_ids:
    ...
    for doc in documents:
        metadata = doc.metadata or {}
        doc_act = (metadata.get("act") or "").strip()
        doc_section = (metadata.get("section_number") or "").strip()
        if act and doc_act != act.strip():
            continue
        if section_number and doc_section != str(section_number).strip():
            continue
        if act or section_number:
            matched.append(doc)
    ...
    matched.sort(
        key=lambda doc: (doc.metadata or {}).get("subsection_number") or ""
    )
    first_meta = matched[0].metadata or {}
...
def retrieve_chunks_by_section(
    ...
    for doc in documents:
        ...
        if section is None:
            continue
        if str(section).strip() != str(section_number).strip():
            continue
        matched.append(doc)
