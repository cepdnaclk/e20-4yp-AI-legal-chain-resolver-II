...
from Tools.retriever_utils import get_vectorstore, retrieve_chunks_title  # noqa: E402
...
def _evaluate_retrieval(
    ...
    for item in items:
        ...
        chunks = retrieve_chunks_title(
            vectorstore, act_name or None, relevant_section
        )
        if not chunks and act_name:
            chunks = retrieve_chunks_title(
...
def main() -> None:
    ...
    default_output = results_dir / "mcq_title_retrieval_results.json"

    parser = argparse.ArgumentParser(
        description="Evaluate MCQ retrieval using title/section lookup only."
    )
    parser.add_argument(
        "--dataset",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Top-k chunks to retrieve (for reporting metrics).",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    print("Title/section retrieval metrics:")
