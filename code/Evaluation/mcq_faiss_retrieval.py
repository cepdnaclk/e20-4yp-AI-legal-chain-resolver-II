...
from Tools.retriever_utils import get_vectorstore, retrieve_chunks_faiss  # noqa: E402
...
def _evaluate_retrieval(
    ...
    for item in items:
        ...
        chunks = retrieve_chunks_faiss(vectorstore, question, top_k)
        retrieved_sections = [
...
def main() -> None:
    ...
    default_output = results_dir / "mcq_faiss_retrieval_results.json"

    parser = argparse.ArgumentParser(
        description="Evaluate MCQ retrieval using FAISS only."
    )
    parser.add_argument(
        "--dataset",
        type=Path,
